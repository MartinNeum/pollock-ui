<template>
    ...
    <v-dialog v-model="dialog" width="auto" persistent>
        <v-card>
            <v-card-text>
                <h1>Glückwunsch!</h1>
                <p>Die Umfrage wurde erfolgreich erstellt!</p>
                <p class="hint"><strong>Hinweis:</strong> Bitte stelle sicher, die Tokens zu speichern. Du wirst sie nicht erneut abrufen können.</p>
            </v-card-text>
            ...
            <v-container>
                ...
                <v-text-field
                    v-model="adminToken"
                    readonly
                    :append-icon="adminTokenCopied ? 'mdi-check' : 'mdi-content-copy'"
                    @click:append="copyAdminToken"
                    variant="outlined"
                ></v-text-field>
            </v-container>
        ...
        store.api.requests.createPoll(title.value, description.value, optionsDic, setting, fixed, owner, personsDic, visibility)
            .then(response => {
                if (response.status === 200) {
                    ...
                    store.methods.addPoll({"title": title.value, "description": description.value, "date": setting.deadline.value, "share": response.data.share.value, "admin": response.data.admin.value})
                    dialog.value = true
                }
            })
            .catch(error => {
    ...
    const copyShareToken = () => {
        navigator.clipboard.writeText(shareToken.value)
            .then(function() {
                ...
                setTimeout(function() {shareTokenCopied.value = false;}, 2000);
                // Erfolgreich kopiert
            })
    };

    const copyAdminToken = () => {
        navigator.clipboard.writeText(adminToken.value)
            .then(function() {
                adminTokenCopied.value = true
                setTimeout(function() {adminTokenCopied.value = false;}, 2000);
                // Erfolgreich kopiert
            })
    };
